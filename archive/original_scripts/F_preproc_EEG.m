...
studyroot = pwd;
addpath([studyroot,'/eeglab2021.1/']);
eeglab nogui;
%xdfpath = [studyroot,'recordings/xdf/']; %original code
xdfpath = [studyroot,'/eeg/']; %for OSF only
chanlocs = [studyroot,'/eeglab2021.1/plugins/dipfit5.3/standard_BESA/standard-10-5-cap385.elp'];
...
for s = 1:length(subjects)
    ...
    bad_chans_out = [outpath,subj,'_bad_chans.csv'];
%% load data
    for b = 1:length(blocks)
        block = blocks{b};
        event_file = [studyroot,'/audio/',subj,'_words2erp_',block,'_trialtime.csv'];

        fixations_file = [studyroot,'/fixations/fixations_times_',block,'_trials.csv'];
        xdfFile = [inpath,'dgame2_',subj,'_Director_',block,'.xdf'];
        mobipath = [inpath,'dgame2_',subj,'_Director_',block,'_MoBI2/'];
        infile = [subj,'_director2.set'];
     
%        %load the data; this is the original version (lines 39-47), line 48 is just for the test data set on OSF
...
%        tmp = mobilab.allStreams.export2eeglab([exportIndex]);
        tmp = pop_loadset('filename',[subj,'_director_',block,'.set'],'filepath',xdfpath);
%% read word data
...
%use clean_raw_data
    ...
    addpath(genpath('/home/ingmar/eeglab2021.1/plugins/cleanline2.00'));
...
%% run amica
    ...
    ica_outdir = '/home/ingmar/Desktop/icatmp/';
