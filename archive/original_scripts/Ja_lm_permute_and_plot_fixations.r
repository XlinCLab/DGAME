# script to perform linear model per 100 ms time bin on fixation-related ERPs with permutation evaluation and FDR correction (Benjamini-Hochberg)
...
load_all_subjects <- function(subjects, base_dir, coords) {
  ...
  all_subject_data <- foreach(
    ...
    pattern <- paste0("^", s, "_[A-Za-z0-9]+_unfold_FIX\\.csv$")
    ...
    s_n <- s_n %>%  
    mutate(fix_time = case_when(
      trial_time < -1 ~ ">1s_before_noun",
      trial_time < 0 & trial_time >= -1    ~ "before_noun",
      trial_time >= 0 & trial_time <= .5 ~ "during_noun",
      trial_time >  .471 & trial_time <= 1   ~ "after_noun",
      TRUE                                ~ ">1s_after_noun"
    )
    )
    s_n
  ...
  dat_fix <- rbindlist(all_subject_data)
  dat_fix
}
...
dat_fix <- load_all_subjects(subjects, base_dir, coords)

dat_fix$condition <- as.factor(dat_fix$condition)
dat_fix$fix_at <- as.factor(dat_fix$fix_at)

dat_fix$laterality <- as.factor(dat_fix$laterality)
dat_fix$saggitality <- as.factor(dat_fix$saggitality)
# Compute Baseline Factor
baseline_a <- dat_fix %>%
  filter(time >= -250, time < 0) %>%
  select(data, subject, condition, laterality, saggitality, fix_time,fix_at) %>% 
  filter(fix_at == 'target')

g_baseline <- baseline_a %>%
  group_by(subject, condition, laterality, saggitality, fix_time,fix_at) %>% 
  filter(fix_at == 'target') %>% 
...
create_time_windows <- function(dat, window_size = 100) {
  ...
  agg <- dat %>%
    group_by(time_bin, laterality, saggitality, condition, fix_time, subject) %>%
    summarize(data_mean = mean(data, na.rm = TRUE), .groups = "drop")
  
  agg_with_baseline <- inner_join(agg, g_baseline,
                                  by = c("subject", "condition", "laterality", "saggitality", "fix_time"))
  agg_with_baseline
}

dat_fix_windows <- create_time_windows(dat_fix)  %>% filter(time_bin >= 0 & time_bin <= 1000)
# Model formula
model_formula <- as.formula("data_mean ~ laterality * saggitality * condition * fix_time * baseline")
...
regression_results <- regression_by_window(dat_fix_windows)
...
block_permutation_test_tstats_fdr <- function(dat_fix_windows,
  ...
  results_list <- lapply(time_bins, function(tb) {
    ...
    formula_str <- if (include_baseline) {
      "data_mean ~ laterality * saggitality * condition * fix_time * baseline"
    } else {
      "data_mean ~ laterality * saggitality * condition * fix_time"
    }
...
# Run block permutation test (optional)
perm_results <- block_permutation_test_tstats_fdr(dat_fix_windows, n_permutations = 20, include_baseline = TRUE)

# Write outputs
fwrite(regression_results, "regression_results_fixations.csv")
fwrite(perm_results, "perm_results_fixations.csv")
...
signif_results_fix = perm_results %>% 
