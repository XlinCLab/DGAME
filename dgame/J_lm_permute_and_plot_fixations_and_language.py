...
class StepJAnalysis:
    ...
    def run(self, mode: str) -> Experiment:
        """Run analysis and plotting for step J in 'FIX' (fixations) or 'N' (nouns/language) mode."""
        mode_label = self.get_mode_label(mode)
        self.logger.info(f"Running permutation tests and plotting statistics for {mode_label}...")
        unfold_out_data = self.load_unfold_out_data(mode=mode)
        ...
        # and for mode="FIX": where the fixation is on the target
        if mode == "FIX":
            groupby = ["subject", "condition", LATERALITY_FIELD, SAGGITALITY_FIELD, "fix_time", "fix_at"]
        elif mode == "N":
            groupby = ["subject", "condition", LATERALITY_FIELD, SAGGITALITY_FIELD, "mean_target_fixation"]
        cols_to_select = ["data"] + groupby
        self.logger.info("Filtering fixation data to establish baseline...")
        baseline_data = (
            unfold_out_data
            # filter time >= -250 & time < 0
            .loc[(unfold_out_data["time"] >= -250) & (unfold_out_data["time"] < 0)]
            # select only these columns
            [cols_to_select]
            # filter fix_at == "target" (only for mode="FIX")
            .pipe(lambda df: df.loc[df["fix_at"] == "target"] if mode == "FIX" else df)
        )
        ...
        self.logger.info(f"Aggregating {mode_label} data by time window...")
        time_windowed_data = create_time_windows(unfold_out_data, mode=mode)
        # Merge with aggregated_baseline data
        if mode == "FIX":
            merge_on_cols = ["subject", "condition", LATERALITY_FIELD, SAGGITALITY_FIELD, "fix_time"]
        elif mode == "N":
            merge_on_cols = ["subject", "condition", LATERALITY_FIELD, SAGGITALITY_FIELD, "mean_target_fixation"]
        time_windowed_data = time_windowed_data.merge(
            baseline_aggregated,
            how="inner",
            on=merge_on_cols
        )
        ...
        time_windowed_data = (
            time_windowed_data
            .loc[(time_windowed_data["time_bin"] >= 0) & (time_windowed_data["time_bin"] <= 1000)]
        )

        # Fit regression models for filtered data by time bin, using all predictors, and collect summaries
        self.logger.info("Fitting regression models on data per time window...")
        time_bin_model_summaries = self.summarize_regression_by_time_bins(time_windowed_data, mode=mode)
        if len(time_bin_model_summaries) > 0:
            ...
            regression_summary_outfile = os.path.join(
                ...
                f"regression_results_{mode_label}.csv",
            )
            time_bin_model_summaries.to_csv(regression_summary_outfile, index=False)
            self.logger.info(f"Wrote regression model summaries to {regression_summary_outfile}")
        else:
            self.logger.warning("No valid regression models were produced.")

        # Run block permutation test
        permutation_results = self.block_permutation_test_tstats_fdr(
            time_windowed_data,
            mode=mode,
        )
        ...
        permutation_results_outfile = os.path.join(
            self.experiment.fixations_outdir,
            f"permutation_results_{mode_label}.csv"
        )
        permutation_results.to_csv(permutation_results_outfile, index=False)
        self.logger.info(f"Wrote permutation test results to {permutation_results_outfile}")

        # Find highest order significant permutation results
        significant_permutation_results = self.find_highest_order_significant_predictor_set(permutation_results)
    ...
    def load_unfold_out_data(self, mode: str) -> pd.DataFrame:
        ...
        per_subject_eeg_outdirs = self.experiment.get_subject_dirs_dict(self.experiment.eeg_outdir)
        ...
        channel_coords = self.experiment.channel_coords

        # Load fixation/language data for all subjects
        self.logger.info(f"Loading EEG {self.mode_label} data for {len(per_subject_unfold_out_dirs)} subject(s)...")
        ...
        for subject, unfold_out_dir in per_subject_unfold_out_dirs.items():
            ...
            self.logger.info("Annotating laterality and saggitality...")
            unfold_out_data = annotate_laterality_and_saggitality(unfold_out_data)

            # Annotate fixation time labels
            if mode == "FIX":
                self.logger.info("Annotating fixation times relative to noun...")
                fix_time_conditions = [
    ...
    def block_permutation_test_tstats_fdr(
            self,
            time_windowed_data: pd.DataFrame,
            mode: str,
        ) -> pd.DataFrame:
        ...
        with tqdm(time_bins, unit="bin") as pbar:
            for time_bin in pbar:
                ...
                pbar.set_description(f"Running block permutation test ({self.n_permutations} permutations) for time_bin={time_bin} ...")

                result = self.time_bin_permutation(
                    time_windowed_data,
                    mode=mode,
                    time_bin=time_bin,
                )
    ...
    def time_bin_permutation(
            self,
            fixation_data_windows: pd.DataFrame,
            time_bin: int,
            mode: str,
        ) -> pd.DataFrame:
        ...
        if design_matrix_rank <= n_predictors:
            self.logger.error(f"Model (time_bin = {time_bin}) is overparametrized: {n_predictors} predictors but only {design_matrix_rank} observations")
            # return
    ...
    def summarize_regression_by_time_bins(
            self,
            fixation_data_windows: pd.DataFrame,
            mode: str
        ) -> pd.DataFrame:
        ...
        for time_window in unique_time_bins:
            ...
            design_matrix_rank = np.linalg.matrix_rank(model.model.exog)
            if design_matrix_rank <= n_predictors:
                self.logger.error(f"Model (time_window = {time_window}) is overparametrized: {n_predictors} predictors but only {design_matrix_rank} observations")
                continue
            model_summary = summarize_stats_model(model)
    ...
    def find_highest_order_significant_predictor_set(
            self,
            df: pd.DataFrame,
            alpha: float = ALPHA,
            predictor_sep: str = ":",
        ...
        for time_bin in unique_time_bins:
            ...
            if len(significant_df) == 0:
                self.logger.info(f"No significant predictors (alpha = {alpha}) found for time bin = {time_bin}")
                continue
            else:
                significant_predictors = significant_df["predictor"].unique()
                n_significant_predictors = len(significant_predictors)
                self.logger.info(f"{n_significant_predictors} significant predictor(s) (alpha = {alpha}) found for time bin = {time_bin}")
