...
def step_J_analysis(experiment: Experiment,
                    mode: str,
                    n_permutations: int = 2000,
    ...
    logger.info(f"Loading EEG {mode_label} data for {len(per_subject_unfold_out_dirs)} subject(s)...")
    all_subjects_data = load_unfold_out_data(
        per_subject_unfold_out_dirs,
    ...
    baseline_data = (
        ...
        .loc[(all_subjects_data["time"] >= -250) & (all_subjects_data["time"] < 0)]
    ...
    logger.info(f"Aggregating {mode_label} data by time window...")
    time_windowed_data = create_time_windows(all_subjects_data, mode=mode)
    ...
    time_bin_model_summaries = summarize_regression_by_time_bins(time_windowed_data, mode=mode)
    ...
    try:
        ...
        logger.info(f"Plotted {mode_label} permutation test results to {plot_outfile}")
