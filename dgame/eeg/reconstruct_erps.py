...
def reshape_beta(
    ...
    """
    Normalise the beta array to [channels x times x params] and return (beta, n_channels, n_params).

    Julia may export coef(model) as a 2D [channels x (params * times)] array.  The coefficient
    layout is parameter-major: all time points for the first coefficient come before all time
    points for the second, and so on.  This function detects that case and reshapes accordingly.
    """
    if beta.ndim == 2:
        ...
        n_channels, _, n_params = beta.shape
    else:
        raise InputValidationError(f"Unexpected beta array shape for subject {subject_id}: {beta.shape}")
    return beta, n_channels, n_params


def extract_coef_metadata(
        uf_meta: dict,
        n_params: int,
        subject_id: str,
    ) -> Tuple[list[str], dict[str, list[str]], list[str]]:
    """Validate and unpack the coefficient metadata exported by the Julia step."""
    ...
    return coef_order, coefnames_by_event, event_order


def reconstruct_noun_erps(
        beta: np.ndarray,
        times: np.ndarray,
        chan_names: list[str],
        coef_order: list[str],
    ...
    """Reconstruct noun ERPs for every channel and write per-channel CSVs."""
...
def reconstruct_fixation_erps(
        beta: np.ndarray,
        times: np.ndarray,
        chan_names: list[str],
        coef_order: list[str],
        coefnames_by_event: dict[str, list[str]],
        events: pd.DataFrame,
        subject_id: str,
        unfold_out_dir: str,
    ) -> None:
    """Reconstruct fixation ERPs for every channel and write per-channel CSVs."""
...
def main(experiment: str | dict | Experiment) -> Experiment:
    ...
    for subject_id, subject_eeg_dir in zip(subject_ids, subject_eeg_dirs):
        ...
        beta, n_channels, n_params = reshape_beta(beta, len(times_s), subject_id)
        ...
        coef_order, coefnames_by_event, event_order = extract_coef_metadata(uf_meta, n_params, subject_id)
        ...
        reconstruct_noun_erps(beta, times, chan_names, coef_order, events, subject_id, unfold_out_dir)
        reconstruct_fixation_erps(beta, times, chan_names, coef_order, coefnames_by_event, events, subject_id, unfold_out_dir)
